...
struct LATE : BidooModule {
	...
	void process(const ProcessArgs &args) override {
		...
		float now = args.sampleTime + (tCurrent > 0.f ? tCurrent : 0.f);
		...
		float timeDiff = tCurrent - tPrevious;
		float lag = rescale(clamp(params[SWING_PARAM].getValue() + params[CVCOEFF_PARAM].getValue() * inputs[SWING_INPUT].getVoltage(), 0.0f, 9.0f), 0.0f, 10.0f, 0.0f, timeDiff);

		if (armed && !odd && ((now - tCurrent) >= lag)) {
			pulse.trigger(1e-3f);
			armed = false;
			odd = true;
		}
